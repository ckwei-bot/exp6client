...
public class AESEncryptClient5 extends ProgClient5 {
    ...
    public AESEncryptClient5(short[] msgIn,int NkIn) {
        msg = msgIn;
    ...
    protected void init() throws Exception {
        AESEncryptCommon.Nk = AESEncryptCommon.ois5.readInt();
        AESEncryptCommon.Nr = AESEncryptCommon.Nk + 6;

        // AESEncryptCommon.initCircuits();

        LookupTableReceiver.ois = AESEncryptCommon.ois5;
    ...
    protected void execTransfer() throws Exception {
        ...
        for (int i = 0; i < slbs.length; i++) {//三个client得到一样的
            slbs[i] = Utils.readBigInteger(bytelength, AESEncryptCommon.ois5);
        }
        ...
        rcver5.execProtocol(m);//每个client都需要执行
        clbs = rcver5.getData();
        StopWatch.taskTimeStamp("receiving labels for self's inputs");
        clbs_own = rcver5.getData();
