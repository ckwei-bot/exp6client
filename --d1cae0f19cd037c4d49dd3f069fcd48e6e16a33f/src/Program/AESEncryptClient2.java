...
public class AESEncryptClient2 extends ProgClient2 {
    ...
    public AESEncryptClient2(short[] msgIn,int NkIn) {
        msg = msgIn;
    ...
    protected void init() throws Exception {
        AESEncryptCommon.Nk = AESEncryptCommon.ois2.readInt();
        AESEncryptCommon.Nr = AESEncryptCommon.Nk + 6;

        // AESEncryptCommon.initCircuits();

        LookupTableReceiver.ois = AESEncryptCommon.ois2;
    ...
    protected void execTransfer() throws Exception {
        ...
        for (int i = 0; i < slbs.length; i++) {//三个client得到一样的
            slbs[i] = Utils.readBigInteger(bytelength, AESEncryptCommon.ois2);
        }
        ...
        rcver2.execProtocol(m);//每个client都需要执行
        clbs = rcver2.getData();
        StopWatch.taskTimeStamp("receiving labels for self's inputs");
        clbs_own = rcver2.getData();
