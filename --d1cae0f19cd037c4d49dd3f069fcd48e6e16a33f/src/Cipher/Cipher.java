...
public final class Cipher {
	...
	BigInteger pad = BigInteger.ZERO;//pad为0
	byte[] tmp = new byte[unitLength / 8];//tmp 为0
	for (int i = 0; i < padLength / unitLength; i++) {
	    System.arraycopy(sha1.digest(), 0, tmp, 0, unitLength/8);//复制digest到tmp数组
	    pad = pad.shiftLeft(unitLength).xor(new BigInteger(1, tmp));//将tmp补到最后
	    sha1.update(tmp);
	}
